...
fn stanza2document(stanzares: NLPServerReturn) -> anyhow::Result<AnnotatedDocument> {
            ...
            for stanzatoken in token_group {
                match stanzatoken.get("id") {
                    Some(Value::Array(subtokenids_arr)) => {
                        let subtokenids = subtokenids_arr
                            .iter()
                        ...
                        children2parent.extend(
                            subtokenids
                                .iter()
                                .map(|stanza_token_id| (*stanza_token_id, parent_idx)),
                        );
                ...
                match stanzatoken.get("id") {
                    ...
                    Some(Value::Number(stanza_token_id_num)) => {
                        ...
                        match children2parent.get(&stanza_token_id) {
                            Some(parent_idx) => {
                                // shadowed, add to parent's shadows
                                intermediate_tokens[*parent_idx].push_shadow(
                                    SentenceConstituent::SubwordToken {
                                        sentence_id: sentence_id,
                                        id: stanza_token_id,
                                        text: stanzatoken_get_text(stanzatoken),
                                        orthography: stanzatoken_get_text(stanzatoken)
                                            .to_lowercase(),
                                ...
                                orthography_set
                                    .insert(stanzatoken_get_text(stanzatoken).to_lowercase());
                                lemma_set.insert(stanzatoken_get_lemma(stanzatoken).to_lowercase());
                            }
                            None => {
                                // not shadowed, add to intermediate_tokens
    ...
    while intermediate_sentences.len() > 0 {
        ...
        match sentence {
            DocumentConstituent::DocumentWhitespace { .. } => sentences.push(sentence),
...
pub async fn tokenise_pipeline(
    ...
    if response.status().is_success() {
        ...
        let res_json: NLPServerReturn = response.json::<NLPServerReturn>().await?;
        dbg!(&res_json);
