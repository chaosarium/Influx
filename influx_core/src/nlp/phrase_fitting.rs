...
pub fn greedy_fit<T: Eq + Hash + Clone, S>(
    seq: Vec<T>,
    trie: &Trie<T, S>,
) -> (Vec<Vec<T>>, Vec<(usize, usize)>) {
...
pub fn recursion_best_fit_prime<T: Eq + Hash + Clone, S>(
    seq: Vec<T>,
    trie: &Trie<T, S>,
) -> ((Vec<Vec<T>>, Vec<(usize, usize)>), usize) {
    if seq.len() == 0 {
        return ((vec![], vec![]), 0);
    }

    let prefixes = trie.search_prefixes_by_ref(&seq, true);
    let max_fitting = prefixes
        .iter()
        .map(|prefix: &Vec<T>| {
            let prefix_len = prefix.len();
...
pub fn recursion_best_fit<T: Eq + Hash + Clone, S>(
    seq: Vec<T>,
    trie: &Trie<T, S>,
) -> (Vec<Vec<T>>, Vec<(usize, usize)>) {
...
pub fn dp_best_fit<T: Eq + Hash + Clone, S>(seq: Vec<T>, trie: &Trie<T, S>) -> Vec<(usize, usize)> {
    ...
    for suffix_len in 1..=seq.len() {
        ...
        let best_sub_prefix_sol = sub_prefixes
            ...
            .min_by_key(|(suffix_slices, suffix_cost)| *suffix_cost)
            .unwrap();
...
mod test {
    ...
    fn test_greedy_fit1() {
        ...
        assert_eq!(
            segments,
            vec![vec![1, 2, 3, 4, 5], vec![6, 7], vec![8], vec![9]]
        );
